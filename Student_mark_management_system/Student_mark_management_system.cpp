...
bool StuList::del(StuNode * pStuNode)
{
	if (pStuNode == NULL)
	{
...
bool StuList::DelFromName(char * name)
{
	return del(stulist.SearchName(name));
}
bool StuList::DelFromID(long id)
{
	return del(stulist.SearchID(id));
}
bool StuList::DelFromID()
{
	return del(stulist.SearchID());
}
StuNode * StuList::Swap(StuNode *p1, StuNode *p2,bool FromMark)
{
	StuNode *temp1;
	StuNode *temp2;
...
StuNode * StuList::SearchName(char * name)
{
	if (name == NULL)
	{
		cout << "请输入学生名字：" << endl;
...
StuNode * StuList::SearchID(long id)
{
	StuNode *p = pHead;
...
double StuList::getClassChineseAverage()
{
	StuNode *p = pHead;
	double ChineseSum = 0;
	int PeopleNum = 0;
...
double StuList::getClassMathAverage()
{
	StuNode *p = pHead;
	double MathSum = 0;
	int PeopleNum = 0;
...
double StuList::getClassEnglishAverage()
{
	StuNode *p = pHead;
	double EnglishSum = 0;
	int PeopleNum = 0;
...
double StuList::getClassChinesePassRate()
{
	StuNode *p = pHead;
	int PeopleNum = 0;
	int PassPeople = 0;
...
double StuList::getClassMathPassRate()
{
	StuNode *p = pHead;
	int PeopleNum = 0;
	int PassPeople = 0;
...
double StuList::getClassEnglishPassRate()
{
	StuNode *p = pHead;
	int PeopleNum = 0;
	int PassPeople = 0;
...
bool TeaList::del(TeaNode * pTeaNode)
{
    if (pTeaNode == NULL)
    {
...
bool TeaList::DelFromName(char *name)
{
    return del(tealist.SearchName(name));
}
void TeaList::ShowAll()
{
    if (pHead == NULL)
    {
        cout << "老师列表为空！" << endl;
...
TeaNode * TeaList::SearchName(char * name)
{
    if (name == NULL)
    {
        cout << "请输入教师名字：" << endl;
